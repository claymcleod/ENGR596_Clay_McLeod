...
let apiKey = "e0c4e7416f066aa7be4671fb0149e28d"
...
class Flickr {
    ...
    func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void){
        ...
        NSURLConnection.sendAsynchronousRequest(searchRequest, queue: processingQueue) {response, data, error in
            ...
            let flickrPhotos : [FlickrPhoto] = photosReceived.map {
                ...
                let flickrPhoto = FlickrPhoto(title: title, photoID: photoID, farm: farm, server: server, secret: secret)
    ...
    private func flickrSearchURLForSearchTerm(searchTerm:String) -> NSURL {
        ...
        let URLString = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=\(apiKey)&text=\(escapedTerm)&per_page=\(FLICKR_APPLICATION_CURRENT_RESULTS_PER_PAGE)&sort=interestingness-desc&format=json&nojsoncallback=1"
